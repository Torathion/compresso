...
describe('last', () => {
  ...
  it('returns the last element when a fallback is provided but array is non-empty', () => {
    expect(last([1, 2, 3], 0)).toBe(3)
    expect(last(['a', 'b', 'c'], 'd')).toBe('c')
    expect(last([true, false, true], false)).toBe(true)
  })

  it('returns the fallback value for an empty array when fallback is provided', () => {
  ...
  it('returns undefined for an empty array when no fallback is provided', () => {
  ...
  it('handles arrays with mixed types when type allows', () => {
