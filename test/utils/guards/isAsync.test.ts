...
describe('isAsync', () => {
  ...
  it('returns false for arrow functions without async', () => {
    const arrowFunc = () => Promise.resolve(42)
    expect(isAsync(arrowFunc)).toBe(false)
  })

  it('returns true for async function with parameters', () => {
    const asyncFunc = async (a: number, b: string) => a + b
    expect(isAsync(asyncFunc)).toBe(true)
  })

  it('returns false for function returning Promise', () => {
  ...
  it('returns true for async method in object', () => {
    const obj = {
      async method() {
        return 42
      }
  ...
  it('returns true for bound async function', () => {
    const asyncFunc = async () => 42
